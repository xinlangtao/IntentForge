...
class ChannelWebhookHttpExchangeHandlerTest {
  @Test
  void shouldRouteTelegramWebhookRequestToInboundProcessor() throws Exception {
  ...
  @Test
  void shouldRejectMissingHookAccount() throws Exception {
    HttpServer server = HttpServer.create(new InetSocketAddress("127.0.0.1", 0), 0);
    try {
      HookHttpRouteRegistrar.register(server, new ChannelWebhookHttpExchangeHandler(
      ...
      HttpResponse<String> response = HttpClient.newHttpClient().send(
          HttpRequest.newBuilder(URI.create(
                  "http://127.0.0.1:" + server.getAddress().getPort()
                      + "/open-api/hooks/channels/telegram/accounts/missing-account/webhook"))
  ...
  @Test
  void shouldRejectUnsupportedHookPath() throws Exception {
    HttpServer server = HttpServer.create(new InetSocketAddress("127.0.0.1", 0), 0);
    try {
      HookHttpRouteRegistrar.register(server, new ChannelWebhookHttpExchangeHandler(
          new ChannelWebhookEndpointController(new InMemoryHookAccountRegistry(), fixedProcessor())));
      ...
      HttpResponse<String> response = HttpClient.newHttpClient().send(
          HttpRequest.newBuilder(URI.create(
                  "http://127.0.0.1:" + server.getAddress().getPort()
                      + "/open-api/hooks/channels/telegram/telegram-account"))
              .POST(HttpRequest.BodyPublishers.ofString("{}"))
              .build(),
          HttpResponse.BodyHandlers.ofString());
